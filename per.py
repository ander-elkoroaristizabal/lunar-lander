...
original code: https://github.com/labmlai/annotated_deep_learning_paper_implementations/blob/master/labml_nn/rl/dqn/replay_buffer.py
---
...
class ReplayBuffer:
    ...
    *Note that we are indexing starting from 1*.
    ...
    def __init__(self, capacity, alpha):
        ...
        # Arrays for buffer
        self.data = {
            'obs': np.zeros(shape=(capacity, 8), dtype=np.uint8),
            'action': np.zeros(shape=capacity, dtype=np.int32),
            'reward': np.zeros(shape=capacity, dtype=np.float32),
            'next_obs': np.zeros(shape=(capacity, 8), dtype=np.uint8),
