...
class DoubleDQNAgentUsingPER:
    ...
    def take_step(self, eps, mode='train'):
        ...
        self.buffer.add(self.state0, action, reward, new_state, terminated)  # guardamos experiencia en el buffer
    ...
    def update(self):
        ...
        batch = self.buffer.sample(batch_size=self.batch_size,
                                   beta=1 - self.epsilon / 2)  # seleccionamos un conjunto del buffer
...
if __name__ == '__main__':
    ...
    agent_name = "double_dqn_w_per"
    agent_title = 'Agente DDQN con PER'
    try:
        os.mkdir(agent_name)
    ...
    ddqn_agent = DoubleDQNAgentUsingPER(
        env=environment,
        dnnetwork=dqn,
        buffer=er_buffer,
    ...
    # Agent training:
    training_time = ddqn_agent.train(
        gamma=GAMMA,
    ...
    # ddqn_agent.dnnetwork.load_state_dict(torch.load(f'{agent_name}/{agent_name}_Trained_Model.pth'))
    # Training evaluation:
    plot_rewards(
        training_rewards=ddqn_agent.training_rewards,
        mean_training_rewards=ddqn_agent.mean_training_rewards,
        reward_threshold=environment.spec.reward_threshold,
        title=agent_title,
        save_file_name=f'{agent_name}/{agent_name}_rewards.png'
    )
    plot_losses(
        training_losses=ddqn_agent.training_losses,
        title=agent_title,
        save_file_name=f'{agent_name}/{agent_name}_losses.png'
    )

    # Saving:
    torch.save(obj=ddqn_agent.dnnetwork.state_dict(), f=f'{agent_name}/{agent_name}_Trained_Model.pth')
    ...
    for i in gif_games:
        render_agent_episode(env_dict=render_env_dict, ag=ddqn_agent, game_seed=eval_games_seed + int(i), eps=eval_eps)
